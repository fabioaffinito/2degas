...
c numero di particelle
      ...
      do it=1,ntypes
       ...
       iplst(it)=i
      enddo
c backflow2 setup (solo e-gas paramagnetico)
...
c write files for contour plot
      implicit none
      include'ewald.h'
...
c parameters for the contour plot
      do it=1,ntypes
       iunit=iunit+1
...
c      if(ntheta.ne.0)elocal=elocal+0.5d0*(p_new(jefp)+p_old(jefp))
       do it=1,ntypes
        if(hbs2m(it).ne.0.d0)then
         do ip=ipfrst(it),iplst(it)
          do idim=1,ndim
...
c age is # of consecutive rejections for a given walker
       age=age+1
       elocal=p_old(jetot)
c      if(ntheta.ne.0)elocal=elocal+p_old(jefp)
      endif
...
c punto iniziale
      ...
      s_new=s
c       do it=1,ntypes
c        h_new(it)=h_new(it)+2*ndim*np(it)*ltf0/dx_bf2**2
c       enddo
      do it=1,ntypes
       h=2*ndim*np(it)*ltf0
       do ip=ipfrst(it),iplst(it)
        do idim=1,ndim
         ...
         g=-ltf
         h=h-ltf
c          g_new(idim,ip)=g_new(idim,ip)-ltf/(2*dx_bf2)
c          h_new(it)=h_new(it)-ltf/dx_bf2**2
         x_new(idim,ip)=x_new(idim,ip)-dx_bf2
         x_new(idim,ip)=x_new(idim,ip)-dx_bf2
         call ltf_bf2(ltf,s,ip)
         g=(ltf+g)/(2*dx_bf2)
...
c intermediate matrix v (F sul Kwon)
         do jf=1,np(jt)
          do lf=1,np(jt)
           do idim=1,ndim
...
c     common /scratch/wrk
c matrix inverse and determinant
c     call zcheck(np(it),morbit,zorb,1)
...
c     call zcheck(np(it),morbit,zorb,2)
c contribution to -log trial function
...
c     if(ntheta.ne.0)then
c      if(alg.eq.'dmc')
c    &   p_new(jpot(0))=p_new(jpot(0))+grad2_ph*hbs2m(it)/npnorm
c     endif
      return
      end
...
c force move of persistent configurations
       p=1.d0
       nage=nage+1
cfn   elseif(s_old*s_new.lt.0.d0)then
c reject move if sign changes (fixnode)
cfn    p=0
c      nsg=nsg+1
      else
...
c variabili locali pw,dpw,ddpw dimensionate da zero
      implicit none
...
c     write(99,*)iinc,veff(it,iinc),qveff(it,iinc)
...
c        ddorb(1,1,i,ipmat)=ddmf(i1)  *pw(i2)
c        ddorb(2,1,i,ipmat)= dmf(i1) *dpw(i2)
c        ddorb(1,2,i,ipmat)= dmf(i1) *dpw(i2)
c        ddorb(2,2,i,ipmat)= amf(i1)*ddpw(i2)
...
c variance minimization using simplex
       itmax_simplex=100
       tol=1.d-6
       d_parm=1.d-2
       do j=1,n_parm_opt
...
c     if(mytid.eq.0)write(6,*)'f1: ans,effpt = ',ans,effpt
      if(mytid.eq.0)then
...
C---------------------------------------------------------------------- 
C     Questa subroutine ordina il vettore A secondo lo schema
C     A(INDEX(1)) < A(INDEX(2)<...<A(INDEX(N1)) ,con INDEX che 
C     va da 1 a N1
C                                                                       
      IMPLICIT REAL*8(A-H,O-Z)                                          
...
c generate 2*nk+1 shifted r.l.vectors
        gnorm2(1)=0.d0
        do idim=1,ndim
...
c jastrow
        ...
        call jastrow(1)
        jprop=jexcite
c generate twisted g-vectors
        do i=1,ntheta         
         do j=1,ntheta         
          call twistg(i,j)
          call ihopa_like(ikho,ikpa,multho,multpa)
c ciclo sulle eccitazioni
...
c calcolo la molteplicita' e gli indici delle buche
      implicit none
      include 'ewald.h'
...
c gvec(idim,jkho) e' il vettore della buca
c gvec(idim,jkpa) e' il vettore della particella
      data i/0/
      save i
      ...
      enddo
c phi e' l'angolo tra k_buca e q 
...
c initialize
      do ip=1,nptot
       ...
       enddo
      enddo
c compute quasi-coordinates qx 
      ijcount=0
      do it=1,ntypes
       i=iubtable(it,it,iinc)
       if(i.ne.0)then
...
c eccitazioni parallele
     ...
     &        *zorb(jk,i)
        ...
        if(jt.eq.itype)call switch(ndim,gvec(1,jkho),gvec(1,jkpa))
c eccitazioni anti-parallele
       elseif(iex.eq.2)then
c mette pa o ho al posto giusto
        if(itype.eq.1)then
